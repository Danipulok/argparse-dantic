...
from argparse_dantic import ArgumentParser, ActionNameBind, BaseModel, Field
...
class MainModel(GlobalModel):
    sub_a: SubAModel = Field(aliases=["sa"], description="Sub A Command")
    sub_b: SubBModel = Field(aliases=["sb"], description="Sub B Command")
...
def main():
    ...
    args = [
        "--g2", # You can put global arguments anywhere in the command line, they will be collected by global_data
        "sa",
        "-o1", "option_1_value",
    ...
    commands(arguments.action_name)(getattr(arguments, arguments.action_name))
    print()
    pprint(arguments)
